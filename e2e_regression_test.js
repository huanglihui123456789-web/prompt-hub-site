...
(async () => {
  ...
  const dataOk = tc.verified === 1194 && tc.synthetic === 0 && tc.native === 555 && tc.community === 0;
  console.log('3) 数据 tier 分布:', JSON.stringify(tc),
    '| 期望 1194/0/555/0:', dataOk ? 'OK' : 'FAIL');
  ...
  const nVer = num();
  console.log('5) 点「已校准/精选」→ 共', nVer, '条 | 期望 1194:', nVer === 1194 ? 'OK' : 'FAIL');

  clickTier('社区/原生'); await sleep(250);
  const nNat = num();
  console.log('6) 点「社区/原生」→ 共', nNat, '条 | 期望 555:', nNat === 555 ? 'OK' : 'FAIL');

  // 回到「全部」→ 1749，且 active 态正确；确认不存在「机器生成」按钮
  ...
  console.log('7) 点「全部」→ 共', nAll, '条 | 期望 1749:', nAll === 1749 ? 'OK' : 'FAIL',
  ...
  const pass = booted && has2 && dataOk && badgeOk && nVer === 1194 &&
    nNat === 555 && nAll === 1749 && allActive && !hasSynBtn && nFr && frBadgeOk;
