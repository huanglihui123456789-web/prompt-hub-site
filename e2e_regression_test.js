...
(async () => {
  ...
  const dataOk = tc.verified === 1222 && tc.synthetic === 0 && tc.native === 548 && tc.community === 0;
  console.log('3) 数据 tier 分布:', JSON.stringify(tc),
    '| 期望 1222/0/548/0:', dataOk ? 'OK' : 'FAIL');
  ...
  const nVer = num();
  console.log('5) 点「已校准/精选」→ 共', nVer, '条 | 期望 1222:', nVer === 1222 ? 'OK' : 'FAIL');

  clickTier('社区/原生'); await sleep(250);
  const nNat = num();
  console.log('6) 点「社区/原生」→ 共', nNat, '条 | 期望 548:', nNat === 548 ? 'OK' : 'FAIL');

  // 回到「全部」→ 1770，且 active 态正确；确认不存在「机器生成」按钮
  ...
  console.log('7) 点「全部」→ 共', nAll, '条 | 期望 1770:', nAll === 1770 ? 'OK' : 'FAIL',
